...
unsigned char Received_Command = 0,Speed_index = 0,Run_Stop_Mode = 0, Mode=0;   //  Run_Stop_Mode -> 0 = Stop , 1 = Run;
...
void loop()
{
  ...
  else if (!radio.available() && Received_Command==0)                               //STOP (all motors stop) , If any other command is received.
  {
    Serial.println("Stop");
    Received_Command = 0;
    Speed_index = 0;
...
/***************** Speed control Logic ********************/
  if (Mode==1){
    ...
    else if(Received_Command == 3)      //circle left (all motors rotate in reverse direction)
    {
      digitalWrite(RIGHT_FORWARD,HIGH);
      digitalWrite(RIGHT_BACKWARD,HIGH);
      digitalWrite(LEFT_FORWARD,LOW);
    ...
    else if(Received_Command == 4)      //circle left (all motors rotate in reverse direction)
    {
      digitalWrite(RIGHT_FORWARD,LOW);
      digitalWrite(RIGHT_BACKWARD,LOW);
      digitalWrite(LEFT_FORWARD,LOW);
